...
module.exports = async (ctx) => {
    ...
    logger.info(count);
    // let loadMoreKey = next === undefined ? {} : JSON.parse(LZString.decompressFromBase64(next));
    ...
    while (query && (query === 'all' || alldata.length < query) && loadMoreKey !== undefined) {
        ...
        logger.info('load more ' + res.data.data.length + res.data.loadMoreKey);
